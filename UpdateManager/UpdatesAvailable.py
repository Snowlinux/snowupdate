...
from .Core.utils import (get_package_label, humanize_size)
...
class UpdatesAvailable(SimpleGtkbuilderApp):
    ...
    def __init__(self, app, header=None, desc=None):
        ...
        pkg_area = CellAreaPackage(bool(self.list.security_groups))
        pkg_column = Gtk.TreeViewColumn.new_with_area(pkg_area)
        pkg_area.column = pkg_column
    ...
    def pkg_toggle_renderer_data_func(self, cell_layout, renderer, model, iter,
                                      data):
        data = model.get_value(iter, LIST_UPDATE_DATA)
        ...
        if data.item:
            ...
            for group in data.groups:
                for item in group.items:
                    pkg = item.pkg
                    this_install = pkg.marked_install or pkg.marked_upgrade
    ...
    def pkg_icon_renderer_data_func(self, cell_layout, renderer, model, iter,
                                    data):
        data = model.get_value(iter, LIST_UPDATE_DATA)
    ...
    def _mark_selected_updates(self):
        def foreach_cb(model, path, iter, data):
            ...
            active = False
            if data.item:
                active = data.item.pkg.marked_install or \
                         data.item.pkg.marked_upgrade
            elif data.group:
    ...
    def update_count(self):
        ...
        self.label_header.set_label(text_header)

        if text_desc is not None:
            self.label_desc.set_label(text_desc)
    ...
    def on_update_toggled(self, renderer, path):
        ...
        if data.groups or data.group:
            if data.groups:
                self.toggle_from_groups(data.groups)
            elif data.group:
                self.toggle_from_groups([data.group])
            self.updates_changed()
            self.treeview_update.queue_draw()
            return
        if (data.item is None or data.item.pkg is None or
                data.item.pkg.name in self.list.held_back):
            return False
        self.setBusy(True)
        # update the cache
        if data.item.pkg.marked_install or data.item.pkg.marked_upgrade:
            data.item.pkg.mark_keep()
            if self.cache._depcache.broken_count:
                Fix = apt_pkg.ProblemResolver(self.cache._depcache)
                Fix.resolve_by_keep()
        else:
            try:
                data.item.pkg.mark_install()
            except SystemError:
                pass
        self.updates_changed()
        self.treeview_update.queue_draw()
    ...
    def toggle_from_groups(self, groups):
        self.setBusy(True)
        actiongroup = apt_pkg.ActionGroup(self.cache._depcache)

        # Deselect all updates if any in group are selected
        keep_packages = any([g.packages_are_selected() for g in groups])
        for group in groups:
            for item in group.items:
                if keep_packages:
                    item.pkg.mark_keep()
                elif not item.pkg.name in self.list.held_back:
                    item.pkg.mark_install(auto_fix=False, auto_inst=False)
    ...
    def restore_state(self):
        ...
        if w > 0 and h > 0 and expanded:
            ...
            GLib.idle_add(lambda: self.window_main.resize(w, h))
        return False
