...
from UpdateManager.UpdateProgress import UpdateProgress
...
class TestBackendError(unittest.TestCase):
    ...
    def test_backend_error(self, update):
        ...
        progress = UpdateProgress(main)
        update.side_effect = lambda: progress.install_backend.emit(
            "action-done", progress.install_backend.UPDATE, True, False,
            "string", "desc")
